...
if [ -f "./nginx-config/.htpasswd" ]; then
...
if [ ! -f "./nginx-config/.htpasswd" ]; then
    ...
    while [ "$auth_success" = false ] && [ $retry_count -lt $max_retries ]; do
        ...
        if echo "$password1" | htpasswd -c -i -B ./nginx-config/.htpasswd admin 2>/dev/null; then
            echo "✅ Basic Auth erfolgreich erstellt mit bcrypt Verschlüsselung!"
            auth_success=true
        elif echo "$password1" | htpasswd -c -i ./nginx-config/.htpasswd admin 2>/dev/null; then
...
echo ""
echo "📁 Datei: ./nginx-config/.htpasswd"
echo "👤 Benutzer: admin"
echo ""
echo "🔧 Weitere Aktionen:"
echo "- Passwort ändern: htpasswd ./nginx-config/.htpasswd admin"
echo "- Benutzer hinzufügen: htpasswd ./nginx-config/.htpasswd BENUTZERNAME"
echo "- Benutzer löschen: htpasswd -D ./nginx-config/.htpasswd BENUTZERNAME"
echo "- Datei anzeigen: cat ./nginx-config/.htpasswd"
echo ""
